      ...
      {activeMix && (
        <div className="grid grid-cols-2 items-center pt-header">
          ...
          <div className="flex flex-col gap-6">
            ...
            {activeMix.description && <Typography type="text">{activeMix.description}</Typography>}
...
// export default function Mixs({ mixs }: { mixs: TypeMix[] }) {
//   const [activeMix, setActiveMix] = useState<TypeMix>(mixs[0]);
//   const [hoveredIndex, setHoveredIndex] = useState<number | null>(null);
//   // const [transitionDiscs, setTransitionDiscs] = useState<TypeMix[]>([
//   //   mixs[0],
//   //   mixs[1],
//   //   mixs[2],
//   //   mixs[3],
//   // ]);
//   const refs = useRef<(HTMLDivElement | null)[]>(new Array(transitionDiscs.length).fill(null));

//   const handleMixClick = (index: number) => {
//     if (!transitionDiscs) return;
//     setActiveMix(mixs[index]);
//     setTransitionDiscs([mixs[index], transitionDiscs[0], transitionDiscs[1], transitionDiscs[2]]);
//     showDisc(refs.current[0]);
//     // nextDisc(refs.current[1]);
//     // nextDisc(refs.current[2]);
//     // nextDisc(refs.current[3]);
//   };

//   const nextDisc = (element: HTMLDivElement | null) => {
//     gsap.to(element, {
...
//   const showDisc = (element: HTMLDivElement | null) => {
//     gsap.fromTo(
//       element,
//       {
...
//               {transitionDiscs.map((transitionDisc, index) => {
//                 return (
//                   <div
//                     key={transitionDisc.title + index}
//                     id={index.toString()}
//                     className="absolute w-full"
//                     style={{ zIndex: 100 - (transitionDiscs.length - index * 10) }}
//                     ref={(el) => (refs.current[transitionDiscs.length - index - 1] = el)}
//                   >
//                     <Vinyle
//                       src={urlForImage(transitionDisc.cover)}
//                       alt={'Mix de ' + transitionDisc.artist}
//                     />
//                   </div>
//                 );
//               })}
